...
mymainmenu = awful.menu({
    items = {{"apps", xdgmenu, "/usr/share/icons/Papirus/22x22/categories/preferences-desktop-icons.svg"},
             {"files", "pcmanfm", "/usr/share/icons/Papirus/22x22/places/user-paleorange-home.svg"},
             {"terminal", terminal, "/usr/share/icons/Papirus/22x22/categories/utilities-terminal.svg"},
             {"standby", "systemctl hybrid-sleep", "/usr/share/icons/Papirus/22x22/status/state_paused.svg"},
...
myTimer:connect_signal("timeout", function()
    battery.updateWidget()
    wifi.updateWidget()
    memory.updateWidget()
    cpu.updateWidget()
    temperature.updateWidget()
end)
myTimer:start()
...
myLongTimer:connect_signal("timeout", function()
    weather.updateWidget()
end)
myLongTimer:start()
...
awesome_grp = gears.table.join(awful.key({modkey}, "s", hotkeys_popup.show_help, {
    ...
    global_wibox_show = not global_wibox_show
    for s in screen do
...
system_grp = gears.table.join(awful.key({}, "XF86AudioRaiseVolume", function()
    volume.inc_vol(5)
end, {
    description = "increase volume (default device)",
    group = "system"
}), awful.key({}, "XF86AudioLowerVolume", function()
    volume.dec_vol(5)
end, {
    description = "decrease volume (default device)",
    group = "system"
}), awful.key({}, "XF86AudioMute", function()
    volume.mute_vol()
end, {
    description = "toggle mute (default device)",
    group = "system"
...
client_grp = gears.table.join(awful.key({modkey}, "j", function()
    awful.client.focus.byidx(1)
end, {
...
launcher_grp = gears.table.join(awful.key({modkey}, "Return", function()
    ...
    awful.spawn(
        'rofi -combi-modi window,drun -font "DejaVu Sans Mono 10" -theme gruvbox-dark-hard -show combi -show-icons')
end, {
    ...
    description = "open internet browser",
    group = "launcher"
}))

layout_grp = gears.table.join(awful.key({modkey}, "l", function()
...
screen_grp = gears.table.join(awful.key({modkey, "Control"}, "k", function()
...
tag_grp = gears.table.join(awful.key({modkey}, "Left", awful.tag.viewprev, {
...
globalkeys = gears.table.join(awesome_grp, system_grp, client_grp, launcher_grp, layout_grp, screen_grp, tag_grp)
