...
function module.weather_status()
    awful.spawn.easy_async_with_shell("curl -H \"Accept-Language: ${LANG%_*}\" \"wttr.in/?M&format=%f %w %p\"",
...
module.weatherwidget:buttons(gears.table.join(awful.button({}, 1, function(t)
    awful.spawn.easy_async_with_shell("curl -H \"Accept-Language: ${LANG%_*}\" \"http://wttr.in/?2MFT\"",
        function(stdout, stderr, reason, exit_code)
            if not stdout then
                module.icon:set_markup("")
                module.widget:set_markup("")
